...
#
# Windows users should use publish.ps1 instead of this script.
#
...
command -v python3 >/dev/null 2>&1 || {
    echo "error: python3 is required to publish a runtime repo" >&2
    exit 1
}
command -v rsync >/dev/null 2>&1 || {
    echo "error: rsync is required by publish.sh (Windows users: use publish.ps1)" >&2
    exit 1
}
...
# --- .gitignore (published instruction files are regenerated by publish.sh/publish.ps1) ---
